...
#else
#define AC_NUM_CONFERENCE_COMMANDS_PYTHON 0
#endif /* PYTHON */
#ifdef QRCODE
#define AC_NUM_CONFERENCE_COMMANDS_QRCODE 1
#else
#define AC_NUM_CONFERENCE_COMMANDS_QRCODE 0
#endif /* QRCODE */
#define AC_NUM_CONFERENCE_COMMANDS (19 + AC_NUM_CONFERENCE_COMMANDS_PYTHON + AC_NUM_CONFERENCE_COMMANDS_QRCODE)

/* Array of conference command names used for tab completion. */
static const char conference_cmd_list[AC_NUM_CONFERENCE_COMMANDS][MAX_CMDNAME_SIZE] = {
    { "/accept"     },
    { "/add"        },
    { "/avatar"     },
    { "/clear"      },
...
{
    free(conferences[conferencenum].name_list);
...
static void conference_onKey(ToxWindow *self, Tox *m, wint_t key, bool ltr)
{
    ChatContext *ctx = self->chatwin;
    ...
    if (key == '\t') {  /* TAB key: auto-completes peer name or command */
        if (ctx->len > 0) {
            ...
            if (ctx->line[0] != L'/' || wcscmp(ctx->line, L"/me") == 0) {
                diff = complete_line(self, conferences[self->num].name_list, conferences[self->num].num_peers,
                                     TOX_MAX_NAME_LENGTH);
                ...
                diff = complete_line(self, conference_cmd_list, AC_NUM_CONFERENCE_COMMANDS, MAX_CMDNAME_SIZE);
            }
...
static void conference_onDraw(ToxWindow *self, Tox *m)
{
    ...
    if (self->show_peerlist) {
        ...
        for (i = 0; i < num_peers && i < maxlines; ++i) {
            ...
            pthread_mutex_lock(&Winthread.lock);
            memcpy(tmpnck, &conferences[self->num].name_list[peer * TOX_MAX_NAME_LENGTH], maxlen);
            pthread_mutex_unlock(&Winthread.lock);
